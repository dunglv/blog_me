...
$full_url = $_protocol.'://'.$_hostname.$temp_uri;
if (in_array('blog_me', explode('/', $full_url))) {
	$home_url = rtrim($_protocol.'://'.$_hostname.'').'/blog_me';
} else {
...
{	
	...
	if(isset($_GET['section'])){
		...
		switch($page){
			...
			$_url = $home_url.'/article/'.$_GET['url'];
			break;
